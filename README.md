...
My list will be a list of books returned from the Open Library API.
The list will show search results based on either a book title or an author name, and each item will include information such as the book title, author
...
searchType – determines whether the user is searching by title or author
searchValue – the book title or author name entered by the user

**3. What is going to be your loop?**
(e.g., loop to display jokes, loop to display forecast days, loop to display movie results)

I will use a loop to go through the array of book results returned by the API.
The loop will display each book’s information on the page, including the title and author, creating a list of results for the user.
